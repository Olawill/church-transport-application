...
export const ScrollPathAnimation = () => {
  ...
  useEffect(() => {
    ...
    // Center container relative to wrapper
    const setCenter = () => {
    ...
    // Timeline for circle following path
    gsap
      .timeline({
        defaults: { ease: "none" },
        scrollTrigger: {
      ...
      .to(circle, { motionPath: path, immediateRender: true }, 0)
    ...
    const ticker = () => {
      xTo(-(gsap.getProperty(circle, "x") as number));
      yTo(-(gsap.getProperty(circle, "y") as number));
    };
  ...
  return (
    <div
      ref={wrapperRef}
      className="relative w-full h-96 md:h-[500px] rounded-xl overflow-hidden bg-gray-100 dark:bg-gray-800"
    >
      <div
        ref={scrollDistRef}
      ...
      >
        <Image
          src="https://assets.codepen.io/721952/mtp.png"
          alt="Map"
          fill
        ...
        <svg
          ...
          style={{ position: "absolute" }}
        >
          <circle ref={circleRef} r={10} fill="#f00" />
          <path
            ref={pathRef}
            stroke="#f00"
            strokeWidth={5}
